  ...
  return (
    <section className="bg-brand-bg-primary py-8 md:py-10">
      <div className="max-w-[1200px] mx-auto px-4 md:px-6">
        <div className="relative w-full overflow-hidden bg-brand-bg-alternate rounded-[6px] py-6 md:py-8 border border-brand-border-dark/60 select-none animate-fade-up-delay-4">
          <div className="flex w-full overflow-hidden">
            ...
            >
              {repeatedItems.map((item, index) => (
                ...
                >
                  {/* Gold/Yellow Badge */}
                  <div className="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-[4px] bg-brand-alternate text-brand-primary flex items-center justify-center transition-transform duration-300 hover:scale-105">
                    <IconRenderer iconName={item.iconName} className="w-5 h-5 md:w-6 md:h-6" />
                  </div>
                  {/* Text Label */}
                  <span className="font-body font-medium text-lg md:text-xl text-brand-bg-secondary uppercase tracking-wider">
                    {item.label}
                  </span>
                </div>
              ))}
